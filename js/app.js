...
***************************************************************************** */
    ...
    $(".questions-contacts__form").on("submit", (function(event) {
        event.stopPropagation();
        event.preventDefault();
        let form = this, data = ($(".submit", form), new FormData), files = $("input[type=file]"), nameInput = $('[name="name"]', form), name = nameInput.val(), phoneInput = $('[name="tel"]', form), phone = phoneInput.val(), phoneRegex = /^(\+?3?8)?(0\d{9}|[1-9]\d{1}-\d{3}-\d{4})$/;
        if ("" === name.trim()) nameInput.addClass("_form-error"); else nameInput.removeClass("_form-error");
        if (!phoneRegex.test(phone)) phoneInput.addClass("_form-error"); else phoneInput.removeClass("_form-error");
        if ("" === name.trim() || !phoneRegex.test(phone)) return false;
        ...
        files.each((function(key, file) {
            let cont = file.files;
            if (cont) $.each(cont, (function(key, value) {
                data.append(key, value);
            }));
        }));
        $.ajax({
            url: "telegramsender/ajax.php",
            type: "POST",
            data,
            cache: false,
            dataType: "json",
